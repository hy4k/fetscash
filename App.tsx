...
function App() {
  ...
  const initializeCategories = async (userId: string) => {
    ...
    if (data && data.length === 0) {
        const defaults = ['FETS Cash Replenishment', 'Office Supplies', 'Travel', 'Food & Beverage', 'Utilities', 'Maintenance', 'Salaries', 'Rent', 'Marketing'];
  ...
  const fetchData = async (userId: string, loc: LocationType) => {
    ...
    const { data: balance } = await supabase.rpc('get_fets_cash_balance', { p_user_id: userId, p_location: loc });
    setCashBalance(balance || 0);
    setLoading(false);
  };
  ...
  const handleSaveExpense = async (expData: Partial<Expense>) => {
    ...
    const isReplenishment = payload.category === 'FETS Cash Replenishment';
       ...
       if (!result.error && result.data) {
           ...
           if (isReplenishment) {
                ...
                const cashPayload = {
                    ...
                    description: `${savedExp.paid_by} ||| FETS Cash Replenishment ||| Replenished from Expense Register`,
                    type: 'replenishment',
