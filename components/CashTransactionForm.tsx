...
import { FetsTransaction, LocationType, Category } from '../types';
...
interface CashTransactionFormProps {
  ...
  onSave: (transaction: Partial<FetsTransaction>) => void;
  onCancel: () => void;
  primaryColor: string;
}
...
export const CashTransactionForm: React.FC<CashTransactionFormProps> = ({
  ...
  useEffect(() => {
    if (transaction) {
      setFormData({
        ...
        description: transaction.clean_description || transaction.description,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Pack data into description field: "ID | Category | Description"
    const packedDescription = `${formData.custom_id} ||| ${formData.category} ||| ${formData.description}`;
    ...
    onSave({
      ...
      type: 'expense', // Always expense for manual entries
    });
  };
